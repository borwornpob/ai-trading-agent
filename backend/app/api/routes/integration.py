...
async def _test_ai_provider() -> dict:
    """Test AI provider connectivity (Claude or OpenAI-compatible)."""
    import os
    provider = settings.ai_provider

    if provider == "openai":
        from mcp_server.agents.base import get_model_orchestrator, get_model_specialist
        if not settings.openai_api_key:
            return {"name": "AI Provider (OpenAI-compatible)", "status": "error", "latency_ms": 0, "detail": "OPENAI_API_KEY not set"}

        label = "OpenAI"
        if settings.openai_base_url:
            label = f"OpenAI-compatible ({settings.openai_base_url})"
        ...
        try:
            ...
            resp = await client.chat.completions.create(
                model=get_model_specialist(),
                messages=[{"role": "user", "content": "Reply OK"}],
                max_tokens=5,
            )
            latency = int((time.time() - start) * 1000)
            return {
                "name": f"AI Provider ({label})",
                "status": "connected",
                "latency_ms": latency,
                "detail": f"Model: {get_model_specialist()}, Orchestrator: {get_model_orchestrator()}",
            }
        except Exception as e:
            return {"name": f"AI Provider ({label})", "status": "error", "latency_ms": 0, "detail": str(e)[:200]}
    else:
        start = time.time()
        token = os.environ.get("CLAUDE_CODE_OAUTH_TOKEN", "")
        if token:
            return {"name": "AI Provider (Claude Max)", "status": "connected", "latency_ms": 0, "detail": "OAuth token configured"}
        return {"name": "AI Provider (Claude Max)", "status": "error", "latency_ms": 0, "detail": "CLAUDE_CODE_OAUTH_TOKEN not set"}
...
def _get_ai_provider_status(claude_token: str) -> str:
    if settings.ai_provider == "openai":
        return "configured" if settings.openai_api_key else "not_configured"
    return "configured" if claude_token else "not_configured"
...
async def test_service(service: str, request: Request):
    ...
    testers = {
        "anthropic": _test_ai_provider,
        "ai": _test_ai_provider,
        "mt5": _test_mt5,
...
async def get_integration_config(db: AsyncSession = Depends(get_db)):
    ...
    return {
        "integrations": [
            {
                "id": "ai_provider",
                "name": f"AI Provider ({settings.ai_provider})",
                "description": "AI for market analysis and autonomous trading decisions (Claude or OpenAI-compatible)",
                "status": _get_ai_provider_status(claude_token),
                "config": {
                    "Provider": settings.ai_provider,
                    "Orchestrator Model": get_model_orchestrator(),
                    "Specialist Model": get_model_specialist(),
                    **({"Base URL": settings.openai_base_url or "default"} if settings.ai_provider == "openai" else {}),
                    **({"Auth": "Max Subscription (OAuth)" if claude_token else "Not configured"} if settings.ai_provider == "claude" else {}),
                    **({"API Key": _mask(settings.openai_api_key)} if settings.ai_provider == "openai" else {}),
                },
                "tools": [
