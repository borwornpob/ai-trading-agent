...
class Settings(BaseSettings):
    ...
    # AI provider: "claude" (Agent SDK, Max subscription) or "openai" (OpenAI-compatible API)
    ai_provider: str = "claude"  # "claude" | "openai"

    # OpenAI-compatible API settings (used when ai_provider="openai")
    # Works with OpenAI, GLM, Ollama, LiteLLM, or any OpenAI-format endpoint
    openai_api_key: str = ""
    openai_base_url: str = ""  # e.g. "https://api.openai.com/v1", "https://open.bigmodel.cn/api/paas/v4"
    openai_model: str = "gpt-4o-mini"  # model for sentiment/analysis (cheap, fast)
    openai_model_orchestrator: str = "gpt-4o"  # model for orchestrator (reasoning)
    ...
    # NOTE: extra="ignore" so unknown env vars (e.g. CLAUDE_CODE_OAUTH_TOKEN,
    # which the Claude Agent SDK reads from the process env directly) don't
    # cause Settings() to raise on startup when loaded from a .env file.
    model_config = {
        "env_file": ".env",
