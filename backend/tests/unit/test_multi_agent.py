...
from mcp_server.agents.base import MODEL_ORCHESTRATOR, MODEL_SPECIALIST, get_model_orchestrator, get_model_specialist
...
class TestModelSelection:
    def test_specialist_uses_haiku(self):
        """Default Claude specialist model is Haiku."""
        assert "haiku" in MODEL_SPECIALIST.lower()

    def test_orchestrator_uses_sonnet(self):
        """Default Claude orchestrator model is Sonnet."""
        assert "sonnet" in MODEL_ORCHESTRATOR.lower()

    def test_getter_returns_openai_model_when_configured(self):
        """get_model_* returns OpenAI models when ai_provider=openai."""
        from app.config import settings
        original = settings.ai_provider
        try:
            settings.ai_provider = "openai"
            assert get_model_specialist() == settings.openai_model
            assert get_model_orchestrator() == settings.openai_model_orchestrator
        finally:
            settings.ai_provider = original

    def test_getter_returns_claude_model_by_default(self):
        """get_model_* returns Claude models when ai_provider=claude."""
        from app.config import settings
        original = settings.ai_provider
        try:
            settings.ai_provider = "claude"
            assert "haiku" in get_model_specialist().lower()
            assert "sonnet" in get_model_orchestrator().lower()
        finally:
            settings.ai_provider = original
...
class TestBaseAgentLoop:
    ...
    async def test_handles_text_response(self):
        ...
        with patch("mcp_server.sdk_client.sdk_agent_loop", AsyncMock(return_value=mock_result)):
